...
function getUserData($conn, $userId) {
    $sql = "SELECT * FROM event_organizer WHERE Organizer_Id = ?";
...
function verifyCurrentPassword($conn, $userId, $currentPassword) {
    $sql = "SELECT Organizer_Password FROM event_organizer WHERE Organizer_Id = ?";
    $stmt = mysqli_prepare($conn, $sql);
    ...
    if ($stmt) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            $user = mysqli_fetch_assoc($result);
            return password_verify($currentPassword, $user['Organizer_Password']);
        } else {
            echo "User not found.";
            exit;
        }
...
function updateEmail($conn, $userId, $newEmail) {
    $sql = "UPDATE event_organizer SET Organizer_Email = ? WHERE Organizer_Id = ?";
    $stmt = mysqli_prepare($conn, $sql);
...
function updatePassword($conn, $userId, $newPassword) {
    ...
    $sql = "UPDATE event_organizer SET Organizer_Password = ? WHERE Organizer_Id = ?";
    $stmt = mysqli_prepare($conn, $sql);
...
function updateUserDetails($conn, $userId, $newName, $newContact) {
    $sql = "UPDATE event_organizer SET Organizer_Name = ?, Organizer_Contact = ? WHERE Organizer_Id = ?";
    $stmt = mysqli_prepare($conn, $sql);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['new-email'])) {
        ...
        if (verifyCurrentPassword($conn, $userId, $currentPassword)) {
            ...
            <form id="contact-info-form" method="post" action="">
                <div class="input-group">
                    <div>
                        ...
                        <input type="text" id="new-name" name="new-name" class="half-width" value="<?php echo htmlspecialchars($user['Organizer_Name']); ?>" disabled>
                    </div>
                </div>
                <div>
                    <label for="new-contact">Cell Phone Number:</label>
                    <input type="text" id="new-contact" name="new-contact" value="<?php echo htmlspecialchars($user['Organizer_Contact']); ?>" disabled>
                </div>
