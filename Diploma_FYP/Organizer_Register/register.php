...
<div class="container">
    ...
    if (isset($_POST["submit"])) {
        ...
        if (empty($fullName) OR empty($email) OR empty($phone) OR empty($password) OR empty($passwordRepeat)) {
        ...
        if (count($errors) > 0) {
            ...
            $sql = "INSERT INTO event_organizer (Organizer_Name, Organizer_Email, Organizer_Contact, Organizer_Password, Organizer_SafetyQuestion, Organizer_SafetyAnswer) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_stmt_init($conn);
            if (mysqli_stmt_prepare($stmt, $sql)) {
                mysqli_stmt_bind_param($stmt, "ssssss", $fullName, $email, $phone, $password, $userSafetyQuestion, $userSafetyAnswer);
                mysqli_stmt_execute($stmt);
    ...
    <h1>
        <span class="welcome-text">Let's Create </span>
        <span class="event-frenzy">Event </span>
    </h1>
